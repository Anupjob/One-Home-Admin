        ...
        async function GetActiveStateList() {
            // await Axios.get(Constant.apiBasePath + 'admin/state/getAll', { headers: { authkey: getAccessToken } })
               ...
               getApiCall('admin/state/getAll')
                .then(data => {
                    console.log('data from api admin/state/getAll', data);
                    if (data.meta.status) {
        ...
        if (allCities.length > 0) {
            ...
            renderAllCities = allCities.map((city, index) => {
                ...
                return <tr key={city._id}>
                    ...
                    <td>{(city.status === 0) ? <span className="text-danger">Deactive</span> : <span className="text-success">Active</span>}</td>
                    <td> <EnableDisable id={city._id} status={city.status} apiurl={`admin/city/update/${city._id}/`} handler={StatusHandler} />
                    <Link to={`/edit-city/${city._id}`}><button className="btn btn-secondary custom-round-icon"><i className="ti-pencil"></i></button></Link>
                </td>
                </tr>;
            })
                        ...
                        <div className="d-flex align-items-center">
                            <nav aria-label="breadcrumb">
                                <ol className="breadcrumb m-0 p-0">
                                    <li className="breadcrumb-item"><Link to="/city-management">City Management</Link>
                                    </li>
                                </ol>
                            </nav>
                        </div>
                    ...
                    <div className="col-12">
                                        ...
                                        <form className="mt-4">
                                            <div className="input-group">
                                                ...
                                                <div className="input-group-append">
                                                    <button className="btn btn-primary" type="button" onClick={(event) => filterHandler(event)}>Filter</button>
                                                </div>
                                                {(isFilterError) ? <div className="invalid-feedback d-block">{errorMessage}</div> : null}
                                            </div>
                        ...
                        <div className="card">
                            <div className="card-body">
                                <div className="row">
                                    <div className="form-group col-md-8 float-right">
                                        <input type="text" id="searchTxt" className="form-control col-6 float-right" defaultValue={searchBoxText} placeholder="Search by city name..." onChange={(event) => searchHandler(event)} />
                                    </div>
                                </div>
                            </div>
                            <div className="table-responsive mb-3">
                                ...
                                <Pagination prev={previous} current={current} next={next} pageCount={totalPage} handler={paginationHandler} />
                            </div>
                        </div>
                    </div>
                </div>
