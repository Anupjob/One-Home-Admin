...
const FaqsCategoryCreate = (props) => {
    ...
    async function getDetials() {
        if (id) {
            setFormType('Edit');
            let response = await getApiCall('common/faq/category/details/' + id);
            if (response.meta.msg && response.data) {
                setData({
    ...
    const Save = async (form_data) => {
        let returnData = await postApiCall('common/faq/category/add', form_data);
        if (returnData.meta.status) {
            swal({text: returnData.meta.msg, icon: "success", timer: 1000})
            history('/faqs/categories')
        }
    }

    const Update = async (form_data) => {
        form_data.categoryId = id;
        let returnData = await postApiCall('common/faq/category/update', form_data);
        if (returnData.meta.status) {
            swal({text: returnData.meta.msg, icon: "success", timer: 1000})
            // history('/amenities')
        }
                ...
                <div className="d-sm-flex align-items-center justify-content-between mb-4">
                    <nav aria-label="breadcrumb">
                                <ol className="breadcrumb m-0 p-0">
                                    <li className="breadcrumb-item"> {formType} FAQ Category
                                    </li>
                                </ol>
                            </nav>
                    <Link to={'/faqs/categories'} className="d-sm-inline-block btn btn-sm btn-warning shadow-sm"><i
...
export default FaqsCategoryCreate
